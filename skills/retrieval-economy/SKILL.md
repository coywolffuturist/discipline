...
Costs below are from one measured query ("<term>", 2026-08-22, on a second machine).
They are ORDERS OF MAGNITUDE, not a fixed ranking — re-measure before quoting.

| question | door | cost |
|---|---|---|
| Does the estate know this word at all? | `corpus grep <term>` | ~1.6 KB, 0 calls |
| Is this retired or superseded? | `corpus check <entity>` | ~1.5 KB, 0 calls |
| What has he ruled on X? | `corpus rulings <subject>`, or SQL | ~1.4 KB, 0 calls |
...
2. **Prefer structure over similarity.** SQL and full-text search are exact
   and free. `corpus check` reads supersession straight from page frontmatter,
