...
# an eighth reviewer: a record tip with an unreviewed PARENT; a held refusal given a new name; the empty record
d, w, bare = repo()
A = head(w); note(w, A, "SURVIVED refuter 2026-09-02"); push(d, w); git(w, "push", "-q", "origin", "refs/notes/reviews")
open(os.path.join(w, "plan.txt"), "w").write("unreviewed\n"); git(w, "add", "-A"); git(w, "commit", "-q", "-m", "U", "--no-verify"); U = head(w)
rec = git(w, "rev-parse", "refs/notes/reviews").stdout.strip()
tree = git(w, "rev-parse", "refs/notes/reviews^{tree}").stdout.strip()
C = subprocess.run(["git", "-C", w, "commit-tree", tree, "-p", rec, "-p", U, "-m", "record"], capture_output=True, text=True, env=GIT_ENV).stdout.strip()
git(w, "update-ref", "refs/notes/reviews", C)
rc, out = push(d, w, "refs/notes/reviews")
bait("BAIT G56 a record tip whose second parent is an unreviewed commit is refused", rc != 0 and "not a notes tree" in out, out)
git(w, "update-ref", "refs/notes/reviews", rec)
bare2 = os.path.join(d, "r2.git"); git(d, "init", "-q", "--bare", bare2); git(w, "remote", "add", "r2", bare2)
C2 = subprocess.run(["git", "-C", w, "commit-tree", tree, "-p", U, "-m", "record"], capture_output=True, text=True, env=GIT_ENV).stdout.strip()
git(w, "update-ref", "refs/notes/reviews", C2)
p4 = subprocess.run(["git", "-C", w, "push", "r2", "refs/notes/reviews"], capture_output=True, text=True, env=dict(GIT_ENV, TMPDIR=d))
bait("BAIT G57 a first publish whose record history holds an unreviewed commit is refused", p4.returncode != 0, p4.stdout + p4.stderr)
git(w, "update-ref", "refs/notes/reviews", rec)
note(w, A, "REFUTED refuter 2026-09-02 found later, after it shipped")
rc, out = push(d, w, A + ":refs/heads/release")
bait("BAIT G58 a commit already on the remote but noted REFUTED is not given a new name", rc != 0 and "REFUTED" in out, out)
git(w, "notes", "--ref=reviews", "remove", A)
rc, out = push(d, w, "refs/notes/reviews")
bait("BAIT G59 a record emptied by removing its only note still publishes (no false denial)", rc == 0, out)
note(w, A, "SURVIVED refuter 2026-09-02"); note(w, U, "SURVIVED refuter 2026-09-02 reviewed after all")
rc, out = push(d, w, "refs/notes/reviews")
...
# an eleventh reviewer: idempotent note operations write EMPTY commits; refusing them locked the record out
d, w, bare = repo()
A = head(w); note(w, A, "SURVIVED refuter 2026-09-02 same text"); push(d, w); git(w, "push", "-q", "origin", "refs/notes/reviews")
note(w, A, "SURVIVED refuter 2026-09-02 same text")           # the reviewer's last act, retried
rc, out = push(d, w, "refs/notes/reviews")
bait("BAIT G68 a retried identical `notes add -f` (an empty commit) still publishes", rc == 0, out)
subprocess.run(["git", "-C", w, "notes", "--ref=reviews", "edit", A], env=dict(GIT_ENV, GIT_EDITOR="true"), capture_output=True)
rc, out = push(d, w, "refs/notes/reviews")
bait("BAIT G69 an unchanged `notes edit` still publishes", rc == 0, out)
c = os.path.join(d, "clone"); subprocess.run(["git", "clone", "-q", bare, c], env=GIT_ENV, check=True)
git(c, "fetch", "-q", "origin", "+refs/notes/reviews:refs/notes/reviews")
open(os.path.join(w, "b.md"), "w").write("b\n"); git(w, "add", "-A"); git(w, "commit", "-q", "-m", "B", "--no-verify"); B = head(w)
note(w, B, "SURVIVED refuter 2026-09-02 identical on both machines")
bnd = os.path.join(d, "b.bundle"); git(w, "bundle", "create", bnd, "HEAD"); git(c, "fetch", "-q", bnd, "HEAD")
git(c, "notes", "--ref=reviews", "add", "-f", "-m", "SURVIVED refuter 2026-09-02 identical on both machines", B)
git(c, "push", "-q", "origin", "refs/notes/reviews")
git(w, "fetch", "-q", "origin", "+refs/notes/reviews:refs/notes/reviews-remote"); git(w, "notes", "--ref=reviews", "merge", "-q", "reviews-remote")
rc, out = push(d, w, "refs/notes/reviews")
bait("BAIT G70 a same-tree `notes merge` from two machines still publishes", rc == 0, out)
open(os.path.join(w, "c.md"), "w").write("c\n"); git(w, "add", "-A"); git(w, "commit", "-q", "-m", "C", "--no-verify")
note(w, head(w), "SURVIVED refuter 2026-09-02")
rc, out = push(d, w)
