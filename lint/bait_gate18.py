...
bait("BAIT G16 a tag at a commit the remote already has is not gated", rc == 0 and "GATE 18" not in out, out)
rc, out = push(d, w)
bait("BAIT G17 an up-to-date push is not gated and burns nothing", rc == 0 and "GATE 18" not in out, out)
shutil.rmtree(d, ignore_errors=True)

d, w, bare = repo(guarded=False)
rc, out = push(d, w)
bait("BAIT G13 an unmarked repo is not gated and prints no gate line", rc == 0 and "gate 18" not in out.lower(), out)
shutil.rmtree(d, ignore_errors=True)
