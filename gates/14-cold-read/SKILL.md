...
Ruled 2026-09-01, after two readers acted wrongly on artifacts that had passed
this gate. Clarity is not one of them — both artifacts were perfectly clear.

1. Would a competent reader **act** wrongly from this?
2. Does a claim depend on a **context the text never states** — which machine,
   which account, which environment? *(One miss: a path true on one host,
   checked on the other, reported false.)*
3. Was this **true when written and now false**? Every self-critical note is a
   claim with a timestamp. *(The other miss: a banner accurate for a week, quoted
   weeks later as current.)*
4. Does anything dead, retired or superseded **read as live**?

## An author re-reading their own work is not a cold read

**Without a context-free reader the row is BLOCKED, not FIRED.** Spawn
`cold-reader` — it starts with none of your context, which is the only property
that matters here.

Measured over one day before this rule: 35 FIRED, 3 N/A, zero BLOCKED, and zero
cold-readers spawned. The gate was reporting health it did not have.
