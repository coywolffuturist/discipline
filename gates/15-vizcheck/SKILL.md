...
## This environment (Coywolf / Mac Mini)

> Environment-specific commands. Replaced per setup in any shared copy.
...
Not to use here:
- ❌ `agent-browser screenshot` — broken upstream (v0.26.0), hangs; shell-intercepted at `~/.zshenv` with a redirect.
- ❌ `tccutil reset` on any Chrome / agent-browser / cua-driver entry — TCC grants are human-set permanent fixtures.

Authenticated real-Chrome flows (rare; banking/travel/social): `agent-browser --profile Default` to interact + `cua-driver call screenshot` to capture. See `reference_browser_routing_by_domain`.
